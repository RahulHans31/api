...
export default async function handler(req, res) {
  try {
    ...
    const bodyText = "A new deal has just been added. Check it out now!";
    const link =
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      console.log(`📤 Deal batch ${i + 1}/${batches.length} sent`);
    }

    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ message: "✅ Deal notifications sent!" }));
  } catch (err) {
